...
[~, xValue1, yValue1] = levenbergConstGamma(f, epsilon, xInput(choosePoint), yInput(choosePoint));
...
xlabel('x')
ylabel('y')
title('z = $x^{5} * e^{-x^{2} - y^{2}}$ with steps for Levenberg-Marquardt Method for gamma = 0.1 for (1,-1)','Interpreter','latex')
...
% Calling the method for constant gamma.
[~, xValue2, yValue2] = levenbergMinGamma(f, epsilon, xInput(choosePoint), yInput(choosePoint));
...
xlabel('x')
ylabel('y')
title('z = $x^{5} * e^{-x^{2} - y^{2}}$ with steps for Levenberg-Marquardt Method for ideal gamma for (1,-1)','Interpreter','latex')
plot_number = plot_number + 1;
...
% Calling the method for constant gamma.
[~, xValue3, yValue3] = levenbergArmijo(f, epsilon, xInput(choosePoint), yInput(choosePoint));
...
xlabel('x')
ylabel('y')
title('z = $x^{5} * e^{-x^{2} - y^{2}}$ with steps for Levenberg-Marquardt Method for Armijo Rule gamma for (1,-1)','Interpreter','latex')
plot_number = plot_number + 1;
